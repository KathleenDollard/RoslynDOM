...
namespace RoslynDom
{
    ...
    public abstract class RDomFactoryHelper
    {
        ...
        private static IEnumerable<TKind> GetFromFactory<TKind>(SyntaxNode syntaxNode, IDom parent, SemanticModel model)
        {
            object testFactory;
    ...
    public abstract class RDomFactoryHelper<TKind> : RDomFactoryHelper
        where TKind : class, IDom
    {
        private IEnumerable<IRDomFactory<TKind>> factories;
        private IRDomFactory<TKind> genericFactory;
        private IEnumerable<Tuple<IRDomFactory<TKind>, Type, Type>> factoryLookup;
        ...
        private IEnumerable<Tuple<IRDomFactory<TKind>, Type, Type>> FactoryLookup
        {
            get
            {
                ...
                {
                    var list = new List<Tuple<IRDomFactory<TKind>, Type, Type>>();
                    foreach (var factory in Factories)
                    {
                        var newTuple = new Tuple<IRDomFactory<TKind>, Type, Type>
        ...
        private Type GetSyntaxType(IRDomFactory<TKind> factory)
        {
        ...
        private Type GetTargetType(IRDomFactory<TKind> factory)
        {
            var factoryType = factory.GetType();
        ...
        protected IRDomFactory<TKind> GetFactory(TKind item)
        {
        ...
        public IEnumerable<TKind> MakeItems(SyntaxNode rawStatement, IDom parent, SemanticModel model)
        {
